...
export default function Product() {
  ...
  useEffect(() => {
    getProduct(slug).then(setProd).catch(console.error);
  }, [slug]);
    ...
    <main className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="mt-4 grid gap-6 md:grid-cols-2">
        <div className="aspect-video bg-stone-100 rounded-lg overflow-hidden">
          <img
            src={prod.imageUrl || 'https://placehold.co/600x400'}
            alt={prod.name}
        ...
        <div>
          ...
          <div className="text-3xl font-extrabold text-brand-800 mt-1">
            {formatARS(prod.price)} <span className="text-base text-stone-500">/ {prod.unit}</span>
          </div>
